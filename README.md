...
| tf-keycloak | [Sources](./AWS/tf-keycloak/)<br>[Details](#tf-keycloak) | You can run Keycloak from the official image without a database and over HTTP.<br>To deploy it, the following environment variables are required: `TF_VAR_AWS_REGION`, `TF_VAR_AWS_ACCOUNT_ID`, `TF_VAR_AWS_ACCESS_KEY_ID`, `TF_VAR_AWS_SECRET_ACCESS_KEY` |
### Details
