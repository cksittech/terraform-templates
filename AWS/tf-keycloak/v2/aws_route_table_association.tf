...
resource "aws_route_table_association" "tf-keycloak-route-table-association-rds-a" {
  subnet_id      = aws_subnet.tf-keycloak-subnet-rds-a.id
  # gateway_id     = ""
  route_table_id = aws_route_table.tf-keycloak-route-table-rds.id
}

resource "aws_route_table_association" "tf-keycloak-route-table-association-rds-c" {
  subnet_id      = aws_subnet.tf-keycloak-subnet-rds-c.id
  # gateway_id     = ""
  route_table_id = aws_route_table.tf-keycloak-route-table-rds.id
}
