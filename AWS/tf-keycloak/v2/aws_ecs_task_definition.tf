resource "aws_ecs_task_definition" "tf-keycloak-ecs-task-definition" {
  container_definitions    = jsonencode([{
    ...
    "environment": [
      ...
      {
        "name": "KC_DB_URL",
        "value": "jdbc:postgresql://${aws_rds_cluster.tf-keycloak-rds-cluster.endpoint}:5432/keycloak"
      },
      {
        "name": "KC_DB_USERNAME",
        "value": "postgres"
  ...
  depends_on = [aws_rds_cluster_instance.tf-keycloak-rds-cluster-instance]
}
